...
GLPanel::GLPanel(QWidget *parent) :
    QGLWidget(parent), c_(), translateDir_(0), objectTranslateDir_(0), objectSpinDir_(0), rotator_(c_)
{
    cont_ = NULL;
    c_.setDefault3D();
    lightPos_.setZero();
...
void GLPanel::keyPressEvent(QKeyEvent *ke)
{
    ...
    case 'D':
        ...
        break;
    case '4':
        objectTranslateDir_ |= TD_LEFT;
        break;
    case '8':
        objectTranslateDir_ |= TD_FWD;
        break;
    case '6':
        objectTranslateDir_ |= TD_RIGHT;
        break;
    case '2':
        objectTranslateDir_ |= TD_BACK;
        break;
    case '7':
        objectSpinDir_ |= TD_FWD;
        break;
    case '9':
        objectSpinDir_ |= TD_BACK;
        break;
    }
}
...
void GLPanel::tick()
{
    ...
    int ofwdamt = 0;
    if(objectTranslateDir_ & TD_FWD)
        ofwdamt++;
    if(objectTranslateDir_ & TD_BACK)
        ofwdamt--;
    Vector3d odir = ofwdamt*center;
    int orghtamt =0 ;
    if(objectTranslateDir_ & TD_LEFT)
        orghtamt--;
    if(objectTranslateDir_ & TD_RIGHT)
        orghtamt++;
    odir += orghtamt*right;
    double odnorm = odir.norm();

    int spinamt = 0;
    if(objectSpinDir_ & TD_FWD)
        spinamt++;
    if(objectSpinDir_ & TD_BACK)
        spinamt--;

    Vector3d spindir = spinamt*center;

    if(odnorm != 0 || spinamt != 0)
    {
        if(odnorm != 0)
            odir /= odnorm;
        else
            odir.setZero();
        QMetaObject::invokeMethod(cont_, "accelerateBody", Q_ARG(double, odir[0]), Q_ARG(double, odir[1]), Q_ARG(double, odir[2]), Q_ARG(double, spindir[0]), Q_ARG(double, spindir[1]), Q_ARG(double, spindir[2]) );
    }
