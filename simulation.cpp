    ...
    renderLock_.lock();
    {
        ...
        for (int i = 1; i <= params_.gridSize; i++) {
            for (int j = 1; j <= params_.gridSize; j++) {
                float val = fluiddensity->valAt(i,j)* 255;
                glColor3f(val, 0, val);
                glVertex3f(.25*(params_.gridSize - i), 0, .25*(j + 5));
            }
        }
...
void Simulation::takeSimulationStep()
{
    ...
    for (int i = 1; i <= params_.gridSize; i++) {
        for (int j = 1; j <= params_.gridSize; j++) {
            fluidvy_prev->valAt(i, j) += params_.gravityG / 10;
            fluidvy->valAt(i, j) += params_.gravityG / 10;
            //fluiddensity_prev->valAt(i, j) = fluiddensity->valAt(i, j);
        }
    }
    set_bnd(params_.gridSize, 2, fluidvy);
    set_bnd(params_.gridSize, 2, fluidvy_prev);
...
void Simulation::set_bnd ( int N, int b, Mat2D *x ) {
    ...
    for ( i=1 ; i<=N ; i++ ) {
        x->valAt(0,i) = b==1 ? -x->valAt(1,i) : x->valAt(1,i);
        x->valAt(N+1,i) = b==1 ? -x->valAt(N,i) : x->valAt(N,i);
        x->valAt(i,0 ) = b==2 ? -x->valAt(i,1) : x->valAt(i,1);
        x->valAt(i,N+1) = b==2 ? -x->valAt(i,N) : x->valAt(i,N);
    }
